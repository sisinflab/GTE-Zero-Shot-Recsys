...
def parse_args():
    ...
    parser.add_argument('--domain', type=str, default='Baby_Products')
    parser.add_argument('--max_his_len', type=int, default=50)
    parser.add_argument('--n_workers', type=int, default=16)
    parser.add_argument('--output_dir', type=str, default='processed/')
    parser.add_argument('--plm', type=str, default='text-embedding-3-large')
...
if __name__ == '__main__':
    ...
    dest_file = os.path.join(output_dir, f'{args.domain}.{args.plm.split("/")[-1]}.feature')
    ...
    if len(existing_embeddings) == len(sorted_text):
        exit("Full embeddings found")

    embeddings = []
    tot_tokens = 0

    with tqdm(enumerate(range(0, len(sorted_text), args.batch_size)), 
              total=(len(sorted_text) + args.batch_size - 1) // args.batch_size, initial=len(existing_embeddings) // args.batch_size) as pbar:

        for bidx, start in pbar:
            batch = sorted_text[start:start + args.batch_size]

            sentences = tokenize_truncate_detokenize(batch, tokenizer)  # truncate to 512 to match blair sentences
            ...
            if (bidx*args.batch_size) % 100 == 0: # save every 100 batches

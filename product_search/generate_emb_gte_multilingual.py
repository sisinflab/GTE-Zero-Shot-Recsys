...
@torch.no_grad
def sentence2emb(args, order_texts, feat_name, tokenizer, model):
    embeddings = []
    ...
    for start in tqdm(range(0, len(order_texts), batch_size)):
        ...
        outputs = outputs.last_hidden_state[:, 0][:768]
        outputs = F.normalize(outputs, p=2, dim=1)
        embeddings.extend(outputs.detach().cpu())
...
def load_plm(model_name='Alibaba-NLP/gte-multilingual-base'):

...
def sentence2emb(args, order_texts, feat_name, model, prompt=None, typ=None):
    ...
    for start in tqdm(range(0, len(order_texts), batch_size)):
        ...
        with torch.no_grad():
            if typ == 'query':
                instruction = "Represent these questions for retrieving supporting documents:"
                outputs = model.encode(sentences, prompt=instruction)
            elif typ == 'item':
                instruction = "Represent these documents for retrieval: "
                outputs = model.encode(sentences, prompt=instruction)
            else:
...
def load_plm(model_name='hkunlp/instructor-xl', device='cuda'):
...
def parse_args():
    ...
    parser.add_argument('--plm_name', type=str, default='hkunlp/instructor-xl')
    parser.add_argument('--feat_name', type=str, default='instructor-xl', help='')
...
if __name__ == '__main__':
    ...
    plm_model = load_plm(args.plm_name)
    print(plm_model)

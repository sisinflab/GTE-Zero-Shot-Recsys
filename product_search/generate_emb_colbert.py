...
def sentence2emb(args, order_texts, feat_name, tokenizer, model, prompt=None):
    ...
    for start in tqdm(range(0, len(order_texts), batch_size)):
        ...
        if args.mode == 'simcse':
            ...
            cls_output = outputs.pooler_output.detach().cpu()
...
def load_plm(model_name='colbert-ir/colbertv2.0'):
...
def parse_args():
    ...
    parser.add_argument('--plm_name', type=str, default='colbert-ir/colbertv2.0')
    parser.add_argument('--feat_name', type=str, default='colbert-base', help='')
